...
k500 = np.argmin(np.absolute(z--500.0))
...
#   S300 = f.variables['timeMonthly_avg_activeTracers_salinity'][0,idx,k300]
   ...
   T = f.variables['timeMonthly_avg_activeTracers_temperature'][0,idx,:]
   Tbot = np.zeros((len(idx),))
   for i in range(len(idx)):
       Tbot[i]=T[i,maxLevelCell[idx[i]]-1]

   dens500=f.variables['timeMonthly_avg_potentialDensity'][0,idx,k500]
...
#      f.close()
   ...
   sc1 = ax0.scatter(yCell[idx], xCell[idx], s=size, c=(dens500), vmin=1027.5, vmax=1027.9, cmap=cmap)
   sc1.cmap.set_under('w')
   i2 = np.nonzero(idx==idxSill)[0][0]
   fig.colorbar(sc1, ax=ax0)
   ax0.plot(yCell[idxSill], xCell[idxSill], 'k.')
   ax0.axis('equal')
   #ax0.set_title("{}:\n year {}, mo {}; density @ 500m".format(path, yr, mo), fontsize=7)
   ax0.set_title("year {}, mo {}; density @ 500m".format(yr, mo))
   plt.axis('off')
